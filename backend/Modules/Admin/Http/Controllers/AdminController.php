            ...
            if (count($app_data) > 0) {
                ...
                if (count($data_component) > 0) {
                    ...
                    foreach ($data_component as $item) {
                        $list_id[] = $item;
                        $list_insert[] = array('app_setting_id' => $id,
                            'component_id' => $item,
                            'order' => $i);
                        $i++;
                    }
...
//        $app_setting->store_user_color = $this->request->input('store_user_color');
    ...
    public function upload()
    {
        ...
        if (count($app_data) > 0) {
            ...
            {
                $upload = new UploadHandler($app_setting->id);
            }
        }
...
//        return $upload;
    ...
    public function uploaddelete($image_id)
    {
        $app_data = App::where('user_id', Auth::user()->id)->first();
        ...
        if (count($app_data) > 0) {
            ...
            if (count($app_setting) > 0)
            {
                $upload = new UploadHandler($app_setting->id,null,false,null);
            }
        }
